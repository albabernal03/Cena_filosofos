...
class Filosofo(threading.Thread): #creamos la clase filosofo
    ...
    def run(self): #metodo run
        while True: #bucle infinito
            ...
            self.cena.actualizar_filosofo(self.id, "Hambriento", 'pink') #actualizamos el estado del filosofo a hambriento
            tenedor_izq_tomado = self.tenedor_izq.tomar(self.id, "izquierdo", 'blue') #tomamos el tenedor izquierdo
            tenedor_der_tomado = self.tenedor_der.tomar(self.id, "derecho", 'blue') #tomamos el tenedor derecho
            if tenedor_izq_tomado and tenedor_der_tomado: #si se han tomado ambos tenedores
                ...
                time.sleep(random.randint(1, 5)) #tiempo aleatorio entre 1 y 5
                self.tenedor_izq.liberar(self.id, "izquierdo", 'grey') #liberamos el tenedor izquierdo
                self.tenedor_der.liberar(self.id, "derecho", 'grey') #liberamos el tenedor derecho
                ...
                if tenedor_der_tomado:
                    self.tenedor_der.liberar(self.id, "derecho", 'grey')
                if tenedor_izq_tomado:
                    self.tenedor_izq.liberar(self.id, "izquierdo", 'grey')
...
class CenaFilosofos:
    def __init__(self):
        ...
        texto_explicativo.place(x=520, y=50)
    ...
    def dibujar_tenedor(self, id,color):
        x, y = POSICIONES_TENEDORES[id]
        self.canvas.create_rectangle(x-10, y-10, x+10, y+10, fill=color, outline='black')

    def actualizar_filosofo(self, id, estado, color):
        self.canvas.delete("filosofo"+str(id)) #borramos el filosofo que estaba antes
    ...
    def actualizar_tenedor(self, id, color):
        self.canvas.delete("tenedor"+str(id))
        self.dibujar_tenedor(id, color)
